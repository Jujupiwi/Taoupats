<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                2013-2014
            </p>
        </blockquote>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Labastidette?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 10</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>JulienG</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 40%;"></div>
                    </div>
                </td>
                <td>40 %</td>
                <td>4 votes</td>
            </tr>
            <tr>
                <td>Alexis</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 20%;"></div>
                    </div>
                </td>
                <td>20 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Zizou</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 10%;"></div>
                    </div>
                </td>
                <td>10 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Enzo</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 10%;"></div>
                    </div>
                </td>
                <td>10 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Pierre</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 10%;"></div>
                    </div>
                </td>
                <td>10 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Dede</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 10%;"></div>
                    </div>
                </td>
                <td>10 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Cadours?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 11</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Zizou</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 27%;"></div>
                    </div>
                </td>
                <td>27 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>Dédé</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 18%;"></div>
                    </div>
                </td>
                <td>18 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Jordan</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 18%;"></div>
                    </div>
                </td>
                <td>18 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>MaximeS</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 18%;"></div>
                    </div>
                </td>
                <td>18 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Pierre</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 9%;"></div>
                    </div>
                </td>
                <td>9 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Alexis</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 9%;"></div>
                    </div>
                </td>
                <td>9 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Labege?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 7</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Paul</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 43%;"></div>
                    </div>
                </td>
                <td>43 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>Johan</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 29%;"></div>
                    </div>
                </td>
                <td>29 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Dédé</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 14%;"></div>
                    </div>
                </td>
                <td>14 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Joffrey</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 14%;"></div>
                    </div>
                </td>
                <td>14 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Roquette?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 19</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Dédé</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 42%;"></div>
                    </div>
                </td>
                <td>42 %</td>
                <td>8 votes</td>
            </tr>
            <tr>
                <td>Alexis</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 37%;"></div>
                    </div>
                </td>
                <td>37 %</td>
                <td>7 votes</td>
            </tr>
            <tr>
                <td>Pierre</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-warning" style="width: 11%;"></div>
                    </div>
                </td>
                <td>11 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>MaximeD</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 5%;"></div>
                    </div>
                </td>
                <td>5 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Johan</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 5%;"></div>
                    </div>
                </td>
                <td>5 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Fenouillet?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 13</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Jordan</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 23%;"></div>
                    </div>
                </td>
                <td>23 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>Dédé</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-warning" style="width: 15%;"></div>
                    </div>
                </td>
                <td>15 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>MaximeS</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-warning" style="width: 15%;"></div>
                    </div>
                </td>
                <td>15 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Paul</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-warning" style="width: 15%;"></div>
                    </div>
                </td>
                <td>15 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Alexy</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Tito</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Joffrey</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Enzo</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Pompertuzat?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 9</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Pierre</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 33%;"></div>
                    </div>
                </td>
                <td>33 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>Alexy</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 22%;"></div>
                    </div>
                </td>
                <td>22 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Florent</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 22%;"></div>
                    </div>
                </td>
                <td>22 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>JulienG</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 22%;"></div>
                    </div>
                </td>
                <td>22 %</td>
                <td>2 votes</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Fonsegrives?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 12</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Zizou</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 50%;"></div>
                    </div>
                </td>
                <td>50 %</td>
                <td>6 votes</td>
            </tr>
            <tr>
                <td>Alexy</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Clement</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Dédé</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Enzo</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Baptiste</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>JulienG</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Labastidette?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 8</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Clement</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 50%;"></div>
                    </div>
                </td>
                <td>50 %</td>
                <td>4 votes</td>
            </tr>
            <tr>
                <td>Jordan</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 25%;"></div>
                    </div>
                </td>
                <td>25 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Zizou</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 13%;"></div>
                    </div>
                </td>
                <td>13 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Baptiste</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 13%;"></div>
                    </div>
                </td>
                <td>13 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Cadours?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 14</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Florent</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 21%;"></div>
                    </div>
                </td>
                <td>21 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>Maxime S</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 21%;"></div>
                    </div>
                </td>
                <td>21 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>Tony</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 14%;"></div>
                    </div>
                </td>
                <td>14 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Alexis</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 14%;"></div>
                    </div>
                </td>
                <td>14 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Joffrey</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 14%;"></div>
                    </div>
                </td>
                <td>14 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Florian</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 7%;"></div>
                    </div>
                </td>
                <td>7 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Zizou</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>6 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Toulouse Entente?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 16</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Alexy</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 56%;"></div>
                    </div>
                </td>
                <td>56 %</td>
                <td>9 votes</td>
            </tr>
            <tr>
                <td>Tony</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 13%;"></div>
                    </div>
                </td>
                <td>13 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Baptiste</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 13%;"></div>
                    </div>
                </td>
                <td>13 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Jordan</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>6 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Zizou</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>6 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Enzo</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>6 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Roquette?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 11</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Baptiste</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 36%;"></div>
                    </div>
                </td>
                <td>36 %</td>
                <td>4 votes</td>
            </tr>
            <tr>
                <td>Maxime P</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 27%;"></div>
                    </div>
                </td>
                <td>27 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>Enzo</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-warning" style="width: 18%;"></div>
                    </div>
                </td>
                <td>18 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Jordan</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 9%;"></div>
                    </div>
                </td>
                <td>9 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Tony</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 9%;"></div>
                    </div>
                </td>
                <td>9 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Fenouillet?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 7</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Maxime P</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 29%;"></div>
                    </div>
                </td>
                <td>29 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Dédé</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 29%;"></div>
                    </div>
                </td>
                <td>29 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Maxime S</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 14%;"></div>
                    </div>
                </td>
                <td>14 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Zizou</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 14%;"></div>
                    </div>
                </td>
                <td>14 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Clement</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 14%;"></div>
                    </div>
                </td>
                <td>14 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Pompertuzat?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 12</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Tony</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 67%;"></div>
                    </div>
                </td>
                <td>67 %</td>
                <td>8 votes</td>
            </tr>
            <tr>
                <td>JulienB</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Tito</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Zizou</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Enzo</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Labege?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 7</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Paul</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 71%;"></div>
                    </div>
                </td>
                <td>71 %</td>
                <td>5 votes</td>
            </tr>
            <tr>
                <td>MaximeP</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 14%;"></div>
                    </div>
                </td>
                <td>14 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Tito</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 14%;"></div>
                    </div>
                </td>
                <td>14 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Leguevin?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 17</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Olivier</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 53%;"></div>
                    </div>
                </td>
                <td>53 %</td>
                <td>9 votes</td>
            </tr>
            <tr>
                <td>Florent</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 18%;"></div>
                    </div>
                </td>
                <td>18 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>Alexy</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>6 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Maxime S</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>6 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Jordan</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>6 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Dédé</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>6 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Florian</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>6 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Quint Fonsegrives?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 22</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Maxime P</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 86%;"></div>
                    </div>
                </td>
                <td>86 %</td>
                <td>19 votes</td>
            </tr>
            <tr>
                <td>Maxime D</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 9%;"></div>
                    </div>
                </td>
                <td>9 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Dédé</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 5%;"></div>
                    </div>
                </td>
                <td>5 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Aussonne?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 12</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Enzo</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 33%;"></div>
                    </div>
                </td>
                <td>33 %</td>
                <td>4 votes</td>
            </tr>
            <tr>
                <td>JulienB</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 25%;"></div>
                    </div>
                </td>
                <td>25 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>Alexy</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-warning" style="width: 17%;"></div>
                    </div>
                </td>
                <td>17 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>MaximeP</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Florian</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Johan</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Montaudran?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 15</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Enzo</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 38%;"></div>
                    </div>
                </td>
                <td>40 %</td>
                <td>6 votes</td>
            </tr>
            <tr>
                <td>Paul</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 25%;"></div>
                    </div>
                </td>
                <td>20 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>JulienB</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-warning" style="width: 13%;"></div>
                    </div>
                </td>
                <td>13 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Tito</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>7 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Olivier</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>7 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Gaetan</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>7 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Florent</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>7 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Nailloux?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 16</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Gaetan</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 38%;"></div>
                    </div>
                </td>
                <td>38 %</td>
                <td>6 votes</td>
            </tr>
            <tr>
                <td>Alexy</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 25%;"></div>
                    </div>
                </td>
                <td>25 %</td>
                <td>4 votes</td>
            </tr>
            <tr>
                <td>MaximeD</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-warning" style="width: 13%;"></div>
                    </div>
                </td>
                <td>13 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>MaximeS</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>6 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Jordan</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>6 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Joffrey</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>6 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Tony</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 6%;"></div>
                    </div>
                </td>
                <td>6 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Cologne?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 10</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Enzo</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 50%;"></div>
                    </div>
                </td>
                <td>50 %</td>
                <td>5 votes</td>
            </tr>
            <tr>
                <td>Tito</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 30%;"></div>
                    </div>
                </td>
                <td>30 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>Dédé</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 10%;"></div>
                    </div>
                </td>
                <td>10 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Laurent</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 10%;"></div>
                    </div>
                </td>
                <td>10 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                2012-2013
            </p>
        </blockquote>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Rodéo?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 12</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Gaetan</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 25%;"></div>
                    </div>
                </td>
                <td>25 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>JB</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-success" style="width: 25%;"></div>
                    </div>
                </td>
                <td>25 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>MaximeP</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-info" style="width: 17%;"></div>
                    </div>
                </td>
                <td>17 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Alexy</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>JulienB</td>
                <td>
                    <div class="progress progress-striped active active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Gerald</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Micka</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre
                Saint-Araille?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 13</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Micka</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-success" style="width: 46%;"></div>
                    </div>
                </td>
                <td>46 %</td>
                <td>6 votes</td>
            </tr>
            <tr>
                <td>JB</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 38%;"></div>
                    </div>
                </td>
                <td>38 %</td>
                <td>5 votes</td>
            </tr>
            <tr>
                <td>Jordan</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Lionel</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 8%;"></div>
                    </div>
                </td>
                <td>8 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre
                Villaudric?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 14</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>J-B</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-success" style="width: 50%;"></div>
                    </div>
                </td>
                <td>50 %</td>
                <td>7 votes</td>
            </tr>
            <tr>
                <td>Jordan</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 14%;"></div>
                    </div>
                </td>
                <td>14 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Tony</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 14%;"></div>
                    </div>
                </td>
                <td>14 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Alexy</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 7%;"></div>
                    </div>
                </td>
                <td>7 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre
                Gardouch?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 21</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>J-B</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-success" style="width: 38%;"></div>
                    </div>
                </td>
                <td>38 %</td>
                <td>8 votes</td>
            </tr>
            <tr>
                <td>Alexy</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-warning" style="width: 14%;"></div>
                    </div>
                </td>
                <td>14 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>Paul</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 10%;"></div>
                    </div>
                </td>
                <td>10 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Max S</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 10%;"></div>
                    </div>
                </td>
                <td>10 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Joffrey</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 10%;"></div>
                    </div>
                </td>
                <td>10 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Jordan</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 5%;"></div>
                    </div>
                </td>
                <td>5 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Max D</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 5%;"></div>
                    </div>
                </td>
                <td>5 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Tony</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 5%;"></div>
                    </div>
                </td>
                <td>5 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Micka</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 5%;"></div>
                    </div>
                </td>
                <td>5 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre
                Labastidette?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 20</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Micka</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-success" style="width: 40%;"></div>
                    </div>
                </td>
                <td>40 %</td>
                <td>8 votes</td>
            </tr>
            <tr>
                <td>Alexy</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-warning" style="width: 20%;"></div>
                    </div>
                </td>
                <td>20 %</td>
                <td>4 votes</td>
            </tr>
            <tr>
                <td>Florent</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 15%;"></div>
                    </div>
                </td>
                <td>15 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>Max S</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 15%;"></div>
                    </div>
                </td>
                <td>15 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>Paul</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 10%;"></div>
                    </div>
                </td>
                <td>10 %</td>
                <td>2 votes</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre
                Toulouse Entente?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 16</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Maxime S</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-success" style="width: 31%;"></div>
                    </div>
                </td>
                <td>31 %</td>
                <td>5 votes</td>
            </tr>
            <tr>
                <td>J-B</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 25%;"></div>
                    </div>
                </td>
                <td>25 %</td>
                <td>4 votes</td>
            </tr>
            <tr>
                <td>Paul</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 25%;"></div>
                    </div>
                </td>
                <td>25 %</td>
                <td>4 votes</td>
            </tr>
            <tr>
                <td>Florent</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 19%;"></div>
                    </div>
                </td>
                <td>19 %</td>
                <td>3 votes</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre
                Auterive?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 13</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Florent</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-success" style="width: 47%;"></div>
                    </div>
                </td>
                <td>46 %</td>
                <td>6 votes</td>
            </tr>
            <tr>
                <td>Tony</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 32%;"></div>
                    </div>
                </td>
                <td>30 %</td>
                <td>4 votes</td>
            </tr>
            <tr>
                <td>Lionel</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 7%;"></div>
                    </div>
                </td>
                <td>7 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>J-B</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 7%;"></div>
                    </div>
                </td>
                <td>7 %</td>
                <td>1 vote</td>
            </tr>
            <tr>
                <td>Joffrey</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 7%;"></div>
                    </div>
                </td>
                <td>7 %</td>
                <td>1 vote</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Paris ou Barcelone?
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 5</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>PSG</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-success" style="width: 60%;"></div>
                    </div>
                </td>
                <td>60 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>BARCELONE</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 40%;"></div>
                    </div>
                </td>
                <td>40 %</td>
                <td>2 votes</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre Rodéo
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 21</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Les Coachs</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-success" style="width: 72%;"></div>
                    </div>
                </td>
                <td>72 %</td>
                <td>15 votes</td>
            </tr>
            <tr>
                <td>L'arbitre</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 28%;"></div>
                    </div>
                </td>
                <td>28 %</td>
                <td>6 votes</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre
                Fronton
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 18</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Alexy</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-success" style="width: 61%;"></div>
                    </div>
                </td>
                <td>61 %</td>
                <td>11 votes</td>
            </tr>
            <tr>
                <td>Jordan</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 11%;"></div>
                    </div>
                </td>
                <td>11 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Enzo</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 11%;"></div>
                    </div>
                </td>
                <td>11 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Tito</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 5%;"></div>
                    </div>
                </td>
                <td>5 %</td>
                <td>1 votes</td>
            </tr>
            <tr>
                <td>Florent</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 5%;"></div>
                    </div>
                </td>
                <td>5 %</td>
                <td>1 votes</td>
            </tr>
            <tr>
                <td>Zizou</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 5%;"></div>
                    </div>
                </td>
                <td>5 %</td>
                <td>1 votes</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre
                Coteaux
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 27</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Maxou D</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-success" style="width: 56%;"></div>
                    </div>
                </td>
                <td>56 %</td>
                <td>15 votes</td>
            </tr>
            <tr>
                <td>Florent</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 30%;"></div>
                    </div>
                </td>
                <td>30 %</td>
                <td>8 votes</td>
            </tr>
            <tr>
                <td>Maxime P</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 7%;"></div>
                    </div>
                </td>
                <td>7 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Olivier</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 7%;"></div>
                    </div>
                </td>
                <td>7 %</td>
                <td>2 votes</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <blockquote>
            <p>
                <i class="icon-signal icon-white"> </i> Homme du match contre
                Villaudric
            </p>
        </blockquote>
    </div>
</div>
<br>
<div class="row-fluid">
    <div class="span12">
        <table
            class="table table-bordered table-striped table-condensed table-hover">
            <thead>
            <tr>
                <th width="20%">Joueur</th>
                <th width="50%">Progression</th>
                <th width="10%">%</th>
                <th width="20%">Nb Votes : 21</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <td>Micka</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-success" style="width: 38%;"></div>
                    </div>
                </td>
                <td>38 %</td>
                <td>8 votes</td>
            </tr>
            <tr>
                <td>Paul</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-warning" style="width: 19%;"></div>
                    </div>
                </td>
                <td>19 %</td>
                <td>4 votes</td>
            </tr>
            <tr>
                <td>Alexy</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 15%;"></div>
                    </div>
                </td>
                <td>15 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>Maxime</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-info" style="width: 15%;"></div>
                    </div>
                </td>
                <td>15 %</td>
                <td>3 votes</td>
            </tr>
            <tr>
                <td>Florent</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 9%;"></div>
                    </div>
                </td>
                <td>9 %</td>
                <td>2 votes</td>
            </tr>
            <tr>
                <td>Julien</td>
                <td>
                    <div class="progress progress-striped active">
                        <div class="bar bar-danger" style="width: 5%;"></div>
                    </div>
                </td>
                <td>5 %</td>
                <td>1 votes</td>
            </tr>
            </tbody>
        </table>
    </div>
</div>
